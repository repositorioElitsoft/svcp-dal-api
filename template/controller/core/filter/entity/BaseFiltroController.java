...
public class #Base#FiltroController {
    ...
    public ResponseEntity<PagedResponse<#Base#>> filtrar(@ModelAttribute #Base#Filtro filtro, PagingAndSorting paginado) {
        logeador.debug("filtrar()");

        try {
            List<#Base#> #base#Lista = #base#FiltroService.filtrar(filtro, paginado);
            int totalFiltro = #base#FiltroService.contarFiltrar(filtro);

            PagedResponse<#Base#> response = PaginationUtils.createPagedResponse(#base#Lista, totalFiltro, paginado);
