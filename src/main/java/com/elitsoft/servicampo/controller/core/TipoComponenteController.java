...
/**
 * Gestiona las peticiones y respuestas http relativas a TipoComponente
 */
@RestController
@RequestMapping("/tipos-componentes")
public class TipoComponenteController {

    @Autowired
    private TipoComponenteService tipocomponenteService;

    private static final Logger logeador = LoggerFactory.getLogger(TipoComponenteController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<TipoComponenteDTO>> agregar(@RequestBody TipoComponenteDTO tipocomponenteDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(tipocomponenteService.agregar(tipocomponenteDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<TipoComponenteDTO> tipocomponenteDTOLote) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<TipoComponenteDTO> tipocomponenteDTOLote) {
...
//        }
    ...
    public ResponseEntity<ApiEnityResponse<TipoComponenteDTO>> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(tipocomponenteService.encontrarPorClave(id))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<TipoComponenteDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(tipocomponenteService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
