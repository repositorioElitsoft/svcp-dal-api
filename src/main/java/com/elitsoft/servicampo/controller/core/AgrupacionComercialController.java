...
public class AgrupacionComercialController {
    ...
    public ResponseEntity<ApiEnityResponse<AgrupacionComercialDTO>> agregar(@RequestBody AgrupacionComercialDTO agrupacionComercialDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(agrupacionComercialService.agregar(agrupacionComercialDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<AgrupacionComercialDTO> agrupacionComercialDTOLote) {
        ...
        try {
            agrupacionComercialService.agregarLote (agrupacionComercialDTOLote);
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<AgrupacionComercialDTO> agrupacionComercialDtoLote) {
        ...
        try {
            agrupacionComercialService.actualizarLote(agrupacionComercialDtoLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<AgrupacionComercialDTO>> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(agrupacionComercialService.encontrarPorClave(id)));  // Retorna  200 OK
        }
        catch (BaseDatosException e) {
    ...
    public ResponseEntity<ApiEnityResponse<List<AgrupacionComercialDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(agrupacionComercialService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.internalServerError().body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
