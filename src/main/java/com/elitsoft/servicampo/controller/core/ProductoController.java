...
/**
 * Gestiona las peticiones y respuestas http relativas a Producto
 */
@RestController
...
public class ProductoController {
    ...
    private static final Logger logeador = LoggerFactory.getLogger(ProductoController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<ProductoDTO>> agregar(@RequestBody ProductoDTO productoDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(productoService.agregar(productoDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<ProductoDTO> productoDTOLote) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<ProductoDTO> productoDTOLote) {
...
//        }
    ...
    public ResponseEntity<ApiEnityResponse<ProductoDTO>> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(productoService.encontrarPorClave(id))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<ProductoDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(productoService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
