 ...
 */
@RestController
@RequestMapping("/tipos-documentos-identificaciones")
public class TipoDocumentoIdentificacionController {
    ...
    public ResponseEntity<ApiEnityResponse<TipoDocumentoIdentificacionDTO>> agregar(@RequestBody TipoDocumentoIdentificacionDTO tipoDocumentoIdentificacionDTO) {
        logeador.debug("agregar() tipodocumentoidentificacion");

        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(tipoDocumentoIdentificacionService.agregar(tipoDocumentoIdentificacionDTO))); // Retorna  201 Created
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<TipoDocumentoIdentificacionDTO> tipoDocumentoIdentificacionDTOLote) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizar(@PathVariable Long id, @RequestBody TipoDocumentoIdentificacionDTO tipoDocumentoIdentificacionDTO) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<TipoDocumentoIdentificacionDTO> tipodocumentoidentificacionDTOLote) {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Mala Peticion - Entrada datos Invalida"),
            @ApiResponse(responseCode = "404", description = "TipoDocumentoIdentificacion no encontrado"),
            @ApiResponse(responseCode = "460", description = "TipoDocumentoIdentificacion Violación de integridad referencial"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor ")
    })
    ...
    public ResponseEntity<ApiEnityResponse<TipoDocumentoIdentificacionDTO>> encontrarPorClave(@PathVariable Long id) {
        logeador.debug("encontrarPorClave(): {}", id);

        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(tipoDocumentoIdentificacionService.encontrarPorClave(id))); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  500 Internal Server Error
        } catch (RecursoNoEncontradoException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  404 Not Found
    ...
    public ResponseEntity<ApiEnityResponse<List<TipoDocumentoIdentificacionDTO>>> obtenerTodos() {
        logeador.debug("obtenerTodos()");

        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(tipoDocumentoIdentificacionService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
