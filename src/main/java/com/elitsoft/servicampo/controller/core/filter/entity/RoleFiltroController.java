...
import com.elitsoft.servicampo.service.core.filter.entity.RoleFiltroService;
...
public class RoleFiltroController {

    @Autowired
    private RoleFiltroService roleFiltroService; //Logica de Negocio de Filtrado del Core Service

    private static final Logger logeador = LoggerFactory.getLogger(RoleFiltroController.class); //Logback

    @GetMapping(value = "/roles", consumes = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Filtra un role", description = "Filtra y hace paginado de role por atributos")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Role Filtrado exitosamente"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor ")
    })
    public ResponseEntity<PagedResponse<RoleDTO>> filtrar(@ModelAttribute RoleFiltro filtro, PagingAndSorting paginado) {
