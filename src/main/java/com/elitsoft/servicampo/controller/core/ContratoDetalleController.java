...
/**
 * Gestiona las peticiones y respuestas http relativas a ContratoDetalle
 */
@RestController
@RequestMapping("/contratos-detalles")
public class ContratoDetalleController {

    @Autowired
    private ContratoDetalleService contratodetalleService;

    private static final Logger logeador = LoggerFactory.getLogger(ContratoDetalleController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDetalleDTO>> agregar(@RequestBody ContratoDetalleDTO contratodetalleDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(contratodetalleService.agregar(contratodetalleDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDetalleDTO>> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalleService.encontrarPorClave(id))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<ContratoDetalleDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalleService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
