...
import com.elitsoft.servicampo.service.mobile.ContratoMobileService;
...
/**
 * Gestiona las peticiones y respuestas http relativas a Contrato para la version mobile
 */
@RestController
@RequestMapping("/mobile/contratos")
public class ContratoMobileController {

    @Autowired
    private ContratoMobileService contratoMobileService;

    private static final Logger logeador = LoggerFactory.getLogger(ContratoMobileController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDTO>> agregar(@RequestBody ContratoDTO contratoDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(contratoMobileService.agregar(contratoDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizar(@PathVariable Long id, @RequestBody ContratoDTO contratoDTO) {
        ...
        try {
            contratoMobileService.actualizar(id, contratoDTO);
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminar(@PathVariable Long id) {
        ...
        try {
            contratoMobileService.eliminar(id);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminarLote(@RequestBody List<ContratoDTO> contratoDTOLote) {
        ...
        try {
            contratoMobileService.eliminarLote(contratoDTOLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDTO>> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratoMobileService.encontrarPorClave(id))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<ContratoDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratoMobileService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
