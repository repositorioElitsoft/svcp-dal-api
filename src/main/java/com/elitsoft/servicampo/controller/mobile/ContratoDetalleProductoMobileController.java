...
import com.elitsoft.servicampo.service.mobile.ContratoDetalleProductoMobileService;
...
/**
 * Gestiona las peticiones y respuestas http relativas a ContratoDetalleProducto para la version mobile
 */
@RestController
@RequestMapping("/mobile/contratos-detalles-productos")
public class ContratoDetalleProductoMobileController {

    @Autowired
    private ContratoDetalleProductoMobileService contratodetalleproductoMobileService;

    private static final Logger logeador = LoggerFactory.getLogger(ContratoDetalleProductoMobileController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDetalleProductoDTO>> agregar(@RequestBody ContratoDetalleProductoDTO contratodetalleproductoDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(contratodetalleproductoMobileService.agregar(contratodetalleproductoDTO))); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<ContratoDetalleProductoDTO> contratodetalleproductoLoteDTO) {
        logeador.debug("agregarLote() contratodetalleproducto");

        try {
            contratodetalleproductoMobileService.agregarLote(contratodetalleproductoLoteDTO); 
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizar(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId, @PathVariable Long correlativoId,
        ...
        try {
            contratodetalleproductoMobileService.actualizar(contratoDetalleId, tipoProductoId, correlativoId, contratodetalleproductoDTO);
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<ContratoDetalleProductoDTO> contratodetalleproductoLoteDTO) {
        logeador.debug("actualizarLote() contratodetalleproducto");

        try {
            contratodetalleproductoMobileService.actualizarLote(contratodetalleproductoLoteDTO);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminar(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId, @PathVariable Long correlativoId) {
        ...
        try {
            contratodetalleproductoMobileService.eliminar(contratoDetalleId, tipoProductoId, correlativoId);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminarLote(@RequestBody List<ContratoDetalleProductoDTO> contratodetalleproductoDTOLote) {
        ...
        try {
            contratodetalleproductoMobileService.eliminarLote(contratodetalleproductoDTOLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDetalleProductoDTO>> encontrarPorClave(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId,
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalleproductoMobileService.encontrarPorClave(contratoDetalleId, tipoProductoId,correlativoId ))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<ContratoDetalleProductoDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalleproductoMobileService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
