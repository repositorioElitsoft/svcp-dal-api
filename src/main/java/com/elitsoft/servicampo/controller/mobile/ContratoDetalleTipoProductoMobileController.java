...
import com.elitsoft.servicampo.service.mobile.ContratoDetalleTipoProductoMobileService;
...
/**
 * Gestiona las peticiones y respuestas http relativas a ContratoDetalleTipoProducto para la version mobile
 */
@RestController
@RequestMapping("/mobile/contratos-detalles-tipos-productos")
public class ContratoDetalleTipoProductoMobileController {

    @Autowired
    private ContratoDetalleTipoProductoMobileService contratodetalletipoproductoMobileService;

    private static final Logger logeador = LoggerFactory.getLogger(ContratoDetalleTipoProductoMobileController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregar(@RequestBody ContratoDetalleTipoProductoDTO contratodetalletipoproductoDTO) {
        ...
        try {
            contratodetalletipoproductoMobileService.agregar(contratodetalletipoproductoDTO);
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<ContratoDetalleTipoProductoDTO> contratodetalletipoproductoLoteDTO) {
        logeador.debug("agregarLote() contratodetalletipoproducto");

        try {
            contratodetalletipoproductoMobileService.agregarLote(contratodetalletipoproductoLoteDTO); 
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizar(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId,
        ...
        try {
            contratodetalletipoproductoMobileService.actualizar(contratoDetalleId, tipoProductoId, contratodetalletipoproductoDTO);
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<ContratoDetalleTipoProductoDTO> contratodetalletipoproductoLoteDTO) {
        logeador.debug("actualizarLote() contratodetalletipoproducto");

        try {
            contratodetalletipoproductoMobileService.actualizarLote(contratodetalletipoproductoLoteDTO);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminar(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId) {
        ...
        try {
            contratodetalletipoproductoMobileService.eliminar(contratoDetalleId, tipoProductoId);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
...
//        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminarLote(@RequestBody List<ContratoDetalleTipoProductoDTO> contratodetalletipoproductoDTOLote) {
        ...
        try {
            contratodetalletipoproductoMobileService.eliminarLote(contratodetalletipoproductoDTOLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  400 Bad Request
...
//        }
    ...
    public ResponseEntity<ApiEnityResponse<ContratoDetalleTipoProductoDTO>> encontrarPorClave(@PathVariable Long contratoDetalleId, @PathVariable Long tipoProductoId) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalletipoproductoMobileService.encontrarPorClave(contratoDetalleId, tipoProductoId ))); // Retorna  200 OK
        }
        catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        } 
    ...
    public ResponseEntity<ApiEnityResponse<List<ContratoDetalleTipoProductoDTO>>> obtenerTodos() {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body(new ApiEnityResponse<>(contratodetalletipoproductoMobileService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ApiEnityResponse<>(null, e.getErrorCode(),  e.getMessage())); // Retorna  500 Internal Server Error
        }
