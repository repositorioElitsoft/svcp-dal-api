...
/**
 * Gestiona las peticiones y respuestas http relativas a DireccionEmpleado para la version mobile
 */
@RestController
@RequestMapping("/mobile/direcciones-empleados")
public class DireccionEmpleadoMobileController {

    @Autowired
    private DireccionEmpleadoMobileService direccionempleadoMobileService;

    private static final Logger logeador = LoggerFactory.getLogger(DireccionEmpleadoMobileController.class); //Logback
    ...
    public ResponseEntity<DireccionEmpleadoDTO> agregar(@RequestBody DireccionEmpleadoDTO direccionempleadoDTO) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(direccionempleadoMobileService.agregar(direccionempleadoDTO)); // Retorna  201 Created
        }
    ...
    public ResponseEntity<String> agregarLote(@RequestBody List<DireccionEmpleadoDTO> direccionempleadoLoteDTO) {
        ...
        try {
            direccionempleadoMobileService.agregarLote(direccionempleadoLoteDTO);
    ...
    public ResponseEntity<String> actualizar(@PathVariable Long id, @RequestBody DireccionEmpleadoDTO direccionempleadoDTO) {
        ...
        try {
            direccionempleadoMobileService.actualizar(id, direccionempleadoDTO);
    ...
    public ResponseEntity<String> actualizarLote(@RequestBody List<DireccionEmpleadoDTO> direccionempleadoLoteDTO) {
        ...
        try {
            direccionempleadoMobileService.actualizarLote(direccionempleadoLoteDTO);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage()); // Retorna  400 Bad Request
    ...
    public ResponseEntity<String> eliminar(@PathVariable Long id) {
        ...
        try {
            direccionempleadoMobileService.eliminar(id);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage()); // Retorna  400 Bad Request
    ...
    public ResponseEntity<String> eliminarLote(@RequestBody List<Long> idLote) {
        ...
        try {
            direccionempleadoMobileService.eliminarLote(idLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage()); // Retorna  400 Bad Request
    ...
    public ResponseEntity<DireccionEmpleadoDTO> encontrarPorClave(@PathVariable Long id) {
        ...
        try {
            DireccionEmpleadoDTO direccionempleadoDTO = direccionempleadoMobileService.encontrarPorClave(id);
            return ResponseEntity.ok(direccionempleadoDTO);  // Retorna  200 OK
        } catch (BaseDatosException e) {
    ...
    public ResponseEntity<List<DireccionEmpleadoDTO>> obtenerTodos() {
        ...
        try {
            direccionempleadoLista = direccionempleadoMobileService.obtenerTodos();
             return ResponseEntity.ok(direccionempleadoLista); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.internalServerError().build(); // Retorna  500 Internal Server Error
        }
