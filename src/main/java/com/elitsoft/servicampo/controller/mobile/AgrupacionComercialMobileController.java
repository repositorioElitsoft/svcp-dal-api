...
/**
 * Gestiona las peticiones y respuestas http relativas a AgrupacionComercial para la version mobile
 */
@RestController
@RequestMapping("/mobile/agrupaciones-comerciales")
public class AgrupacionComercialMobileController {

    @Autowired
    private AgrupacionComercialMobileService agrupacionComercialMobileService;

    private static final Logger logeador = LoggerFactory.getLogger(AgrupacionComercialMobileController.class); //Logback
    ...
    public ResponseEntity<ApiEnityResponse<AgrupacionComercialDTO>> agregar(@RequestBody AgrupacionComercialDTO agrupacionComercialDTO) {
        logeador.debug("agregar() agrupacioncomercial");

        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(new ApiEnityResponse<>(agrupacionComercialMobileService.agregar(agrupacionComercialDTO), null, "AgrupacionComercial agregado exitosamente")); // Retorna  201 Created
        }
    ...
    public ResponseEntity<ApiEnityResponse<String>> agregarLote(@RequestBody List<AgrupacionComercialDTO> agrupacionComercialDTOLote) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizar(@PathVariable Long id, @RequestBody AgrupacionComercialDTO agrupacionComercialDTO) {
    ...
    public ResponseEntity<ApiEnityResponse<String>> actualizarLote(@RequestBody List<AgrupacionComercialDTO> agrupacionComercialDTOLote) {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Mala Peticion - Entrada datos Invalida"),
            @ApiResponse(responseCode = "404", description = "AgrupacionComercial no encontrado"),
            @ApiResponse(responseCode = "460", description = "AgrupacionComercial Viola integridad referencial"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor ")
    })
    public ResponseEntity<ApiEnityResponse<String>> eliminar(@PathVariable Long id) {
        ...
        try {
            agrupacionComercialMobileService.eliminar(id);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<String>> eliminarLote(@RequestBody List<Long> idLote) {
        ...
        try {
            agrupacionComercialMobileService.eliminarLote(idLote);
            return ResponseEntity.noContent().build(); // Retorna  204 No Content
        }
        catch (EntradaInvalidadException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  400 Bad Request
    ...
    public ResponseEntity<ApiEnityResponse<AgrupacionComercialDTO>> encontrarPorClave(@PathVariable Long id) {
        logeador.debug("encontrarPorClave(): {}", id);

        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(agrupacionComercialMobileService.encontrarPorClave(id)));  // Retorna  200 OK
        } catch (BaseDatosException e) {
    ...
    public ResponseEntity<ApiEnityResponse<List<AgrupacionComercialDTO>>> obtenerTodos() {
        logeador.debug("obtenerTodos()");

        try {
            return ResponseEntity.ok(new ApiEnityResponse<>(agrupacionComercialMobileService.obtenerTodos())); // Retorna  200 OK
        } catch (BaseDatosException e) {
            return ResponseEntity.internalServerError().body(new ApiEnityResponse<>(null, e.getErrorCode(), e.getMessage())); // Retorna  500 Internal Server Error
        }
    }
