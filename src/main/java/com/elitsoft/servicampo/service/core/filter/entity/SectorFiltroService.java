...
@Service
public class SectorFiltroService {
    ...
    /**
     * Cuenta los registros que coinciden con el filtro dinamico para Sector
     * @param filtro clase que tiene los atributos a filtrar
     * @return int cantidad de registros que retorna el filtro
     */
    public int contarFiltrar(SectorFiltro filtro) throws BaseDatosException {
        logeador.debug("contarFiltrar()");

        try {
            return sectorMapper.contarFiltrar(filtro);
        }  catch (DataAccessException e) {
            logeador.error("{}, {}, {}",Constantes.SECTOR_FILTRAR_MENSAJE,  filtro.toString(), e, e);
            throw new BaseDatosException(Constantes.SECTOR_FILTRAR_MENSAJE, e);
        }
    }
}