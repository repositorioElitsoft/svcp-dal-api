...
public class TrabajoFiltroService {
    ...
    /**
     * Ejecuta filtro dinamico y paginacion para Trabajo y Asignaciones de Tareas
     *
     * @param filtro   clase que tiene los atributos a filtrar
     * @param paginado clase que tiene los atributos de paginacion
     * @return List<TrabajoDTO> lista de entidades Trabajo
     * @throws BaseDatosException si la entrada LotePaginado tiene errores.
     */
    public List<TrabajoDTO> filtrarAsignacion(TrabajoFiltro filtro, PagingAndSorting paginado) throws BaseDatosException {
        logeador.debug("filtrarAsginacion()");

        try {
            int desplazamiento = paginado.getPageNumber() * paginado.getPageSize();

            return trabajoMapper.filtrarAsignacion(filtro, paginado.getSortField(), paginado.getSortDirection(), paginado.getPageSize(), desplazamiento);
        } catch (DataAccessException e) {
            logeador.error("{}, {}, {}, {}", Constantes.TRABAJO_FILTRAR_MENSAJE, filtro.toString(), paginado.toString(), e, e);
            throw new BaseDatosException(Constantes.TRABAJO_FILTRAR_MENSAJE, e);
        }
    ...
    /**
     * Cuenta los registros que coinciden con el filtro dinamico para Trabajo y Asignaciones de Tareas
     *
     * @param filtro clase que tiene los atributos a filtrar
     * @return int cantidad de registros que retorna el filtro
     */
    public int contarFiltrarAsignacion(TrabajoFiltro filtro) throws BaseDatosException {
        logeador.debug("contarFiltrarAsginacion()");

        try {
            return trabajoMapper.contarFiltrarAsignacion(filtro);
        } catch (DataAccessException e) {
            logeador.error("{}, {}, {}", Constantes.TRABAJO_FILTRAR_MENSAJE, filtro.toString(), e, e);
            throw new BaseDatosException(Constantes.TRABAJO_FILTRAR_MENSAJE, e);
        }
