...
@Service
public class TipoServicioFiltroService {
    ...
    private static final Logger logeador = LoggerFactory.getLogger(TipoServicioFiltroService.class); //Logback

    /** Ejecuta filtro dinamico y paginacion para TipoServicio
     * @param filtro clase que tiene los atributos a filtrar
     * @param paginado clase que tiene los atributos de paginacion
     * @return List<TipoServicioDto> lista de entidades TipoServicio
     * @throws BaseDatosException si la entrada LotePaginado tiene errores.
     */
    public List<TipoServicioDto> filtrar(TipoServicioFiltro filtro, PagingAndSorting paginado) throws BaseDatosException {
    ...
    /**
     * Cuenta los registros que coinciden con el filtro dinamico para TipoServicio
     * @param filtro clase que tiene los atributos a filtrar
     * @return int cantidad de registros que retorna el filtro
     */
    public int contarFiltrar(TipoServicioFiltro filtro) throws BaseDatosException {
        logeador.debug("contarFiltrar()");

        try {
            return tiposervicioMapper.contarFiltrar(filtro);
        }  catch (DataAccessException e) {
            logeador.error("{}, {}, {}",Constantes.TIPOSERVICIO_FILTRAR_MENSAJE,  filtro.toString(), e, e);
            throw new BaseDatosException(Constantes.TIPOSERVICIO_FILTRAR_MENSAJE, e);
        }
    }
}